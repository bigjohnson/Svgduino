...
static const MyTinyWebServer::MimeType text_html_content_type = 4;

MyTinyWebServer::MyTinyWebServer(PathHandler handlers[],
			     const char** headers)
  : handlers_(handlers),
...
void MyTinyWebServer::begin() {
  server_.begin();
}

// Process headers.
boolean MyTinyWebServer::process_headers() {
  if (headers_) {
...
boolean MyTinyWebServer::is_requested_header(const char** header) {
  if (!headers_) {
    return false;
  }
  for (int i = 0; headers_[i].header; i++) {
...
boolean MyTinyWebServer::assign_header_value(const char* header, char* value) {
  if (!headers_) {
    return false;
  }
  boolean found = false;
...
void MyTinyWebServer::send_error_code(Client& client, int code) {
#if DEBUG
...
void MyTinyWebServer::send_content_type(MimeType mime_type) {
...
void MyTinyWebServer::send_content_type(const char* content_type) {
  client_ << content_type_msg;
  client_.println(content_type);
}

const char* MyTinyWebServer::get_path() { return path_; }

const MyTinyWebServer::HttpRequestType MyTinyWebServer::get_type() {
  return request_type_;
}

const char* MyTinyWebServer::get_header_value(const char* name) {
  if (!headers_) {
    return NULL;
  }
  for (int i = 0; headers_[i].header; i++) {
...
char* MyTinyWebServer::decode_url_encoded(const char* s) {
  if (!s) {
    return NULL;
  }
...
char* MyTinyWebServer::get_file_from_path(const char* path) {
...
MyTinyWebServer::MimeType MyTinyWebServer::get_mime_type_from_filename(
    const char* filename) {
...
void MyTinyWebServer::send_file(SdFile& file) {
  size_t size;
...
size_t MyTinyWebServer::write(uint8_t c) {
  client_.write(c);
}

size_t MyTinyWebServer::write(const char *str) {
  client_.write(str);
}

size_t MyTinyWebServer::write(const uint8_t *buffer, size_t size) {
  client_.write(buffer, size);
}

boolean MyTinyWebServer::read_next_char(Client& client, uint8_t* ch) {
  if (!client.available()) {
    return false;
  } else {
    *ch = client.read();
...
boolean MyTinyWebServer::get_line(char* buffer, int size) {
  int i = 0;
  char ch;
...
char* MyTinyWebServer::get_field(const char* buffer, int which) {
  char* field = NULL;
...
int read_chars(MyTinyWebServer& web_server, Client& client,
...
boolean put_handler(MyTinyWebServer& web_server) {
